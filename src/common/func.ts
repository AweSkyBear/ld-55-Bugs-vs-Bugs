...
import { setGlobalVar, debugLog, exposeToWindow } from './debug'
export {
  omit,
  either,
  any,
...
exposeToWindow({ deepMerge, merge: mergeDeepRight })
...
export const getUIElementActualPos = (el: GameObject & Zone): Point => {
  const { offsetX, offsetY } = compose(
    defaultTo({ offsetX: 0, offsetY: 0 }),
    path(['_anchor'])
  )(el) as { offsetY; offsetX }
...
export const defer = (cb: Func<any, any>, timeout?: number) => setTimeout(cb, timeout)
