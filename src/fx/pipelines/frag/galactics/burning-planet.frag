// https://glslsandbox.com/e#64237.0

/*
 * Original shader from: https://www.shadertoy.com/view/4lf3Rj
 */
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
	...
	vec3 from = vec3(0., 0., -2. + texture(iChannel0, uv * .0 + iTime).x * stepsize); //from+dither
	...
	for(float r = 10.; r < steps; r++) {
		...
		//loop continues because of previous problems with breaks and not always optimizes much
			l = pow(max(.53, dot(normalize(p), normalize(vec3(-1., .5, -0.3)))), 4.) * (.5 + texture(iChannel1, uv * vec2(2., 1.) * (1. + p.z * .5) + vec2(tx + t * .5, 0.)).x * 2.);
	}
	v /= steps;
	v *= brightness; // average values and apply bright factor
