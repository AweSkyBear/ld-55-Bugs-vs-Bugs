// ORIGINAL: http://glslsandbox.com/e#73363.0

#ifdef GL_ES
precision highp float; // WOW !!!!!!!! -> THIS FIXES THE ISSUE WTIH SOME DEVICES, e.g. A MOBILE PHONE 
#endif
...
void main() {   
  ...
  from -= position;
	...
	// from.x += transverseSpeed*(1.0)*cos(0.001*time) + 0.0001*time;
	// from.y += transverseSpeed*(1.0)*sin(0.001*time) +0.0001*time;
	// from.z += 0.003*time;	
	...
	float zooom = (time2-1.)*speed; // NOTE: ORIGINAL WAS 3311
	from += forward* zooom;
